...
void CMatrix::RotationX(float r) {
	m11 = static_cast<float>(cos(r));
	m22 = static_cast<float>(cos(r));
	m12 = static_cast<float>(-sin(r));
	m21 = static_cast<float>(sin(r));
}
void CMatrix::RotationY(float r) {
	m00 = static_cast<float>(cos(r));
	m22 = static_cast<float>(cos(r));
	m20 = static_cast<float>(-sin(r));
	m02 = static_cast<float>(sin(r));
}
void CMatrix::RotationZ(float r) {
	m00 = static_cast<float>(cos(r));
	m11 = static_cast<float>(cos(r));
	m01 = static_cast<float>(-sin(r));
	m10 = static_cast<float>(sin(r));
}

void CMatrix::RotationAxis(float vx, float vy, float vz, float r) {
	float sine = static_cast<float>(sin(r));
	float cosine = static_cast<float>(cos(r));
...
void CMatrix::Perspective(float fovy, float aspect, float zNear, float zFar) {
	float f = static_cast<float>(1.0 / tan((static_cast<double>(fovy) / 2.0)));
	m00 = f / aspect;
	m11 = f;
